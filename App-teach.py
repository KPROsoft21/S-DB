...
class Student:
    def __init__(self,root):
        ...
        title=Label(self.root,text="SOT's Teacher Management System",bd=10,relief=GROOVE, font=('cambria',40,"bold"),bg="royal blue",fg="white")
        ...
        Manage_Frame=Frame(self.root,bd=4,relief=RIDGE,bg="royal blue")
        Manage_Frame.place(x=20,y=100,width=450,height=580)
        m_title=Label(Manage_Frame,text="Manage Teachers",bg="royal blue",fg="white",font=('times new roman',30,"bold"))
        m_title.grid(row=0,columnspan=2,pady=20)

        lbl_roll=Label(Manage_Frame,text="Register No",bg="royal blue",fg="white",font=('times new roman',20,"bold"))
        lbl_roll.grid(row=1,column=0,pady=10,padx=20, sticky="w")

        txt_roll=Entry(Manage_Frame,textvariable=self.Roll_No_var,font=('times new roman',15,"bold"),bd=5,relief=GROOVE)
        txt_roll.grid(row=1,column=1,pady=10,padx=20, sticky="w")

        lbl_name = Label(Manage_Frame, text="Name", bg="royal blue", fg="white",
        ...
        lbl_dept = Label(Manage_Frame, text="Department", bg="royal blue", fg="white",
                         font=('times new roman', 20, "bold"))
        ...
        lbl_email = Label(Manage_Frame, text="Email", bg="royal blue", fg="white",
                         font=('times new roman', 20, "bold"))
        ...
        lbl_Gender = Label(Manage_Frame, text="Gender", bg="royal blue", fg="white",
                         font=('times new roman', 20, "bold"))
        ...
        lbl_contact = Label(Manage_Frame, text="Contact", bg="royal blue", fg="white",
                         font=('times new roman', 20, "bold"))
        ...
        lbl_dob = Label(Manage_Frame, text="D.O.B", bg="royal blue", fg="white",
                         font=('times new roman', 20, "bold"))
        ...
        #===Button===#
        btn_Frame = Frame(Manage_Frame, bd=4, relief=RIDGE, bg="royal blue")
        btn_Frame.place(x=15, y=515, width=420)

        Addbtn=Button(btn_Frame,text="Add",width=10,command=self.add_teachers,bg="indigo",fg="white").grid(row=0,column=0,padx=10,pady=10)
        ...
        Detail_Frame = Frame(self.root, bd=4, relief=RIDGE, bg="royal blue")
        ...
        Table_Frame= Frame(Detail_Frame, bd=4, relief=RIDGE, bg="royal blue")
    ...
    def add_teachers(self):
        if self.Roll_No_var.get()=="" or self.name_var.get()=="":
            messagebox.showerror("Error","All fields are required SOT!!!")
        else:
            con=pymysql.connect(host="localhost",user="root",password="root",database="SDBS")
            cur=con.cursor()
            ...
            self.clear()
            con.close()
            messagebox.showinfo("Success","Records has been inserted SOT!!!")
    def fetch_data(self):
        con = pymysql.connect(host="localhost", user="root", password="root", database="SDBS")
        cur = con.cursor()
    ...
    def update_data(self):
        con = pymysql.connect(host="localhost", user="root", password="root", database="SDBS")
        cur = con.cursor()
        cur.execute("update teachers set name=%s,dept=%s,email=%s,gender=%s,contact=%s,dob=%s where roll_no=%s", (
            self.name_var.get(),
            self.dept_var.get(),
            self.email_var.get(),
            self.gender_var.get(),
    ...
    def delete_data(self):
        con = pymysql.connect(host="localhost", user="root", password="root", database="SDBS")
        cur = con.cursor()
        cur.execute("delete from teachers where roll_no=%s",self.Roll_No_var.get())
        con.commit()
        con.close()
        self.fetch_data()
        self.clear()
